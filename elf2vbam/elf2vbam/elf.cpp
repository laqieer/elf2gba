...
int load_elf(char *szFile)
{
	...
	if (CPUIsELF(szFile)) {
		...
		std::string imageName(szFile);
		std::ofstream image(imageName + ".gba", std::ios::out | std::ios::binary);
		image.write(reinterpret_cast<char *>(whereToLoad), romSize);
		image.close();
		return 1;
	}
	else
	{
		systemMessage(MSG_ERROR_OPENING_IMAGE, N_("Error %s doesn't have .elf extension"),
			szFile);
		return 0;
	}
}